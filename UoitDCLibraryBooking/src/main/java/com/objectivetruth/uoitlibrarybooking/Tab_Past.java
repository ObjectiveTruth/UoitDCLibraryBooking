...
public class Tab_Past extends Fragment {
	final String TAG = "Tab_Past";
	GridView gridView;
    ...
    public void LoginResults(MyAccountLoginResultEvent event) {
        if(!event.errorMessage.isEmpty() && event.result != null) {
            ...
            String[] arrayToUseTempTemp = event.result.get(2);
    ...
    private void updateArrayToUseFromDb(){
        ...
        Cursor c = db.query(MainActivity.mdbHelper.MY_BOOKINGS_TABLE_NAME, new String[]{MainActivity.mdbHelper.PAST_BOOKINGS}, null, null, null, null, null);
        //Log.i(TAG, "count is " + String.valueOf(c.getCount()));

        if((c.moveToNext() == false) || (c.getString(c.getColumnIndex(MainActivity.mdbHelper.PAST_BOOKINGS)) == null )){
            arrayToUse = new String[0];
        }
        else{
            ArrayList<String> arrayToUseTemp = new ArrayList<String>();
            ...
            arrayToUseTemp.add(c.getString(c.getColumnIndex(MainActivity.mdbHelper.PAST_BOOKINGS)));
            //int i = 1;
            String toAdd;
            while(c.moveToNext()){
                toAdd = c.getString(c.getColumnIndex(MainActivity.mdbHelper.PAST_BOOKINGS));
