...
public class Tab_Complete extends Fragment {
	final String TAG = "Tab_Complete";
	GridView gridView;
	String[] arrayToUse = new String[0];
	SQLiteDatabase db;
    ChangeColorGridAdapter adapter;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
    	...
    	adapter = new ChangeColorGridAdapter(getActivity(),
				arrayToUse);
...
public void onCreate(Bundle savedInstanceState) {
    ...
    public void LoginResults(MyAccountLoginResultEvent event) {
        if(!event.errorMessage.isEmpty() && event.result != null) {
            ...
            String[] arrayToUseTempTemp = event.result.get(1);
    ...
    private void updateArrayToUseFromDb(){
        ...
        Cursor c = db.query(MainActivity.mdbHelper.MY_BOOKINGS_TABLE_NAME, new String[]{MainActivity.mdbHelper.COMPLETED_BOOKINGS}, null, null, null, null, null);
        //Log.i(TAG, "count is " + String.valueOf(c.getCount()));

        if((c.moveToNext() == false) || (c.getString(c.getColumnIndex(MainActivity.mdbHelper.COMPLETED_BOOKINGS)) == null )){
            arrayToUse = new String[0];
        }
        else{
            ArrayList<String> arrayToUseTemp = new ArrayList<String>();
            ...
            arrayToUseTemp.add(c.getString(c.getColumnIndex(MainActivity.mdbHelper.COMPLETED_BOOKINGS)));
            //int i = 1;
            int j = 2;
            String toAdd;
            while(c.moveToNext()){
                toAdd = c.getString(c.getColumnIndex(MainActivity.mdbHelper.COMPLETED_BOOKINGS));
                if(toAdd != null && !toAdd.isEmpty()){
                    if((j % 4) == 0){
                        arrayToUseTemp.set(j-2, arrayToUseTemp.get(j-2) + "\n-" + toAdd);
                    }
                    else{
            ...
            for(int j = 0; j < arrayToUseTempSize; j ++){
                if(((j+1) % 4) == 0){
                    arrayToUseTemp.set(j, arrayToUseTemp.get(j) + " - " + arrayToUseTemp.get(j+1));
                    arrayToUseTemp.remove(j+1);
                }
